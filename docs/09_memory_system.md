...
- 同一活跃上下文累计记忆默认 32,768 字节；已注入的 AGENTS、skill、kernel、index 和其他记忆都必须计入。初始化记录的是已加载内容字节；宿主封装由完整请求计量承担。
- 2026-09-09 用户授权 agent 在额度影响工作时自主调整：`memory_gate.py resize --root . --session 原账本ID --context-bytes 196608 --reason '具体工作理由'`。上限 262,144 字节，单包仍为 12,288；原账本 used、去重记录和历史不清零，调整记录时间、旧值、新值与原因。禁止缩到已用额度以下。默认额度不足不再是暂停实现/合并工作的理由。
