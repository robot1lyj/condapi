...
| 跨主题恢复摘要 | kernel.md，注明 canonical 来源 | 只在确有恢复缺口时读取，可从 owner 重建 |
| 路由、任务操作边界 | index、一个相关 mode | owner 未知或需要具体操作边界时才选择 |
| 详细事实和操作条件 | 编号化 docs | 按决策选择完整章节 |
| 候选经验、证据引用和依赖 | docs/cache/records/*.json | 按项目、平台、合同、版本过滤 |
| 恢复目标、已完成步骤、未知项、下一步 | docs/cache/working.md（确需跨轮交接时才创建） | 用一个小型 checkpoint 替换旧进度，完成后清理热入口 |
...
通用 skill 中不存服务器地址、训练结果或项目事实副本。维护唯一 owner 后，仅在跨主题恢复确需该事实时替换 kernel 的来源导航；普通进度不写 kernel。历史记录不自动成为当前默认。长期资料不设行数硬限制；将低频细节移出默认加载路径，不删除证据或未完成事项。

## 按任务检索

2026-09-14 用户进一步要求记忆技能强制分层，防止热记忆无序扩张。共享 `mlops-memory` 已增加热项准入、项目文件/默认加载路径预算、按主题替换、低频细节降级及写回后计量/续作检查，方法见 [冷热分层](../skills/mlops-memory/references/layers.md)。长期证据无行数上限不代表kernel可以无限增长；各项目在自己的记忆owner声明预算，不能把YAM的具体设备或产品规则复制为通用skill事实。本轮修改技能规则，未宣称宿主已安装自动压缩或字节门禁。

2026-09-24 取消固定的 `AGENTS → kernel → index → mode` 启动链：宿主已注入 `AGENTS.md` 时，它就是默认项目记忆；不要再读一次。已知 owner 直接读其相关章节，未知 owner 才查 index，跨主题恢复才查 kernel，具体操作边界需要时只选一个 mode。`CLAUDE.md` 仅转发到 `AGENTS.md`，不存第二份默认模型状态。任何索引、模式和历史文件都不能因为存在就自动预载。

**热路径预算（完整 UTF-8 文件字节）：** `AGENTS.md` ≤12 KiB，`CLAUDE.md` ≤1 KiB，kernel ≤3 KiB，index ≤5 KiB，每个 mode ≤2 KiB。默认路径只包含 `AGENTS.md`，上限 12 KiB；若宿主同时注入 `CLAUDE.md`，合计上限 13 KiB。可选路由按需叠加，预算不限制详细 owner、报告、原始证据或累计读取。修改预算须在本页记录原因和前后实测，不可因一次追加而自动抬高；超限应先替换旧摘要或降级低频项。

本次审计前，旧固定链的 `AGENTS`、kernel、index 共 **30,820 字节**，若再选最大 mode 为 32,652 字节；`CLAUDE.md` 另有 1,334 字节且重复预载、含过时 LoRA 默认说法。整理后四文件分别为 10,706、480、2,130、4,164 字节；默认只读 AGENTS，比旧三文件固定链少 20,114 字节（约 65%）。这是**文件/默认检索字节**，不是实际请求 token；宿主注入行为与现存对话不会被仓库文字自动压缩。完整案例恢复仍需语义核对。

**新增记忆文件门槛：** 先检索现有 owner、报告和索引；普通进度直接替换 owner 的当前状态，不新建日报、摘要副本或带日期的记忆文件。只有既有 owner 无法承载的独立证据/合同才新建文件，并在 owner 留入口及适用范围。`docs/07_change_log.md` 只记改变决策的原因和结果，不作为逐次运行日志；旧事故、旧实验和原始日志保留为冷证据。修改路由时检查旧链接，删除热摘要前确认安全条件可从 `AGENTS.md` 或 owner 找回。

本次静态恢复审阅覆盖三个下一步：XR-1 checkpoint/环境由 `AGENTS.md` 指向 02/10，再按需取安装报告；Pi DAgger 配方先由 `AGENTS.md` 的逐轮确认规则拦住，再取 03；Thor 真机动作先见 `AGENTS.md` 的左臂异常提醒，再取 136000 报告和 08。上述是路由与含义检查，不是实际任务回放、训练/部署验收或上下文 token 实测。

先明确下一项决策、缺失事实与约束，再选择资料；已注入或仍在上下文中的内容不重读。只展开相关 owner 的必要章节，发现缺口、冲突或验收需要时再展开相连原文，不预加载所有参考资料和历史。
...
下一步已有充分依据时停止检索并继续执行；缺少必要事实时有目的地展开。确需跨轮交接时才将已完成工作整理为 `docs/cache/working.md`，替换而非逐次追加；普通进度直接写 canonical owner 或留在任务历史，不产生新的 dated memory 文件。写摘要不会移除既有消息，只有宿主实际压缩/替换上下文才改变保留内容。
