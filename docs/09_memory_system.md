...
2026-09-24 取消固定的 `AGENTS → kernel → index → mode` 启动链，并删除冗余的 `CLAUDE.md`：宿主已注入 `AGENTS.md` 时，它就是唯一默认项目记忆，不再读一次。已知 owner 直接读其相关章节，未知 owner 才查 index，跨主题恢复才查 kernel，具体操作边界需要时只选一个 mode。任何索引、模式和历史文件都不能因为存在就自动预载。

**热路径预算（完整 UTF-8 文件字节）：** `AGENTS.md` ≤9 KiB，kernel ≤3 KiB，index ≤5 KiB，每个 mode ≤2 KiB。默认路径只包含 `AGENTS.md`，上限 9 KiB；可选路由按需叠加。预算不限制详细 owner、报告、原始证据或累计读取。修改预算须在本页记录原因和前后实测，不可因一次追加而自动抬高；超限先替换旧摘要或降级低频项。

原固定三文件链 `AGENTS`、kernel、index 共 **30,820 字节**；上轮先取消自动预载，本轮删除冗余入口并将重复命令移至既有环境 owner。现 `AGENTS.md` 8,145 字节、kernel 2,130 字节、index 4,164 字节；默认文件比原固定链少 22,675 字节（约 74%）。这只衡量默认读取的文件字节，**不是**实际请求 token 或宿主压缩效果；历史细节仍按任务展开。
