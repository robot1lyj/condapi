...
**最新测试优先：乐高分拣3（14:14会话）。** 用户补充已优化为异步推理、关节动作改善但夹爪不能闭合，指定查看“乐高分拣3”。本轮经只读状态接口定位到 `session_20260916_141405_f47544/episode_000001`，只取记录文件，未操作机械臂、相机或控制实现。已核对源端/本地HDF5哈希一致，完整分析见 [最新证据](reports/training/redesign-20260916/README.md#乐高分拣3最新异步测试)。下方90帧属于旧版本，不覆盖本段。
...
因此P0优先于新训练：固定该次优化版本，核对每个请求的原始H50、observation时刻、选中索引、策略值、记录中的提交目标和反馈。在安全条件由YAM侧另行核验持久的已知归一化夹爪目标与物理行程；本分析本身不发运动命令。模型侧固定输入/噪声消融与低延迟参考对照用于区分策略重规划和执行转换，不能用延长整块盲执行掩盖问题。先处理录制队列溢出，后续完整任务试验才有可用证据。
...
### 数据方案：保留全量，建立两个可复现的小集合

以下为拟议实验，未实际筛选episode或生成训练集；不删除、裁帧或覆盖原始数据。
...
| 顺序 | 工作 | 初始预算与结束条件 |
|---|---|---|
| P0 | 固定100000；核对轨迹/图像/噪声/时间合同 | 优先1个工作日；交付可归因的离线记录，GPU前向仅在Thor/获准服务器；本地不跑训练 |
| P1 | 建D0/D1及固定验证；Pi D0拟合检查 | D0先1k，必要到3k，约0.6–1.9小时；不改善先查合同，不直接加到100k |
| P2 | Pi0.5 base→D1全量微调基线 | 2k/5k/10k检查，初始上限10k；验证与任务趋势改善后再决定至20k。新优化器/调度覆盖本实验预算，不沿用162097步长周期；LR以小预算验证确定 |
| P3 | Evo-1同D1对照 | 先验YAM batch/保存重载/Thor原生推理，再测服务器单卡/多卡吞吐；stage1在1k/3k/5k检查，stage2在2k/5k/10k检查，均为试验预算而非收敛保证 |
| P4 | 选路线、扩充数据 | 以同场景成功率和取得结果的GPU小时决策；两者均接近但不抓时优先补失败阶段证据，暂不做新全量长训 |

同4卡资源串行安排实验，不假定Pi和Evo能同时占满四卡。P2/P3同时报告样本暴露量与GPU小时，Evo两个stage成本都计入、batch区分每卡与全局；不以相同步数当公平算力对照，不直接比较两种loss。旧100000为全量数据训练的业务基线，不是与D1 Evo受控的数据消融。
...
本轮建议为“Pi0.5保留基线，Evo-1做优先轻量对照”，未自动切模型、回退Pi LoRA、开训或同步服务器/Thor代码。
