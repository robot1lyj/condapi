...
## 2026-09-21 · 乐高 DAgger 首轮方案

用户确认每轮固定从20h RTC模型M0初始化，采集使用当前已验收的最好模型；累计原D0及各轮合格专家纠正，不使用上一轮适配权重作为默认初始化。首轮约50次有效纠正按组划分约40train/10val；规划50/50与25/75两组混采对照，固定更新预算，并以独立真机对比选择。具体采集规程、步数公式、实验卡、实施缺口及论文证据统一见 [DAgger方案手册](reference/lego_dagger_playbook.md)，数据字段归 [04](04_data_contracts.md#dagger-纠正数据发布合同2026-09-21方案)。

状态是方案已记录，未实现双池采样/有效起点过滤，未采集、训练、部署或验证M1。M0实际checkpoint与服务状态在采集前冻结，不根据20h准备配置推断已上线。后续轮次50/25/25仅为候选，不是已确定最优比例。
