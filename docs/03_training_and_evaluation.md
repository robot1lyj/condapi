...
## 2026-09-17 · Qwen3-VL-2B与独立RLT记忆研究方案

用户确定新研究路线为 `Qwen/Qwen3-VL-2B-Instruct` + 独立 Recurrent Looped Transformer 记忆 + Evo风格连续动作头，第一数据源为YAM ABC-130k，并确认RLT为2026-09-12提出、9月15日修订的那篇报告。详细设计、固定来源、序列数据、训练预算、消融和Thor验收归 [完整方案](reference/qwen3_vl_rlt_abc130k_plan.md)。该路线是新的研究policy，尚未实现、下载新权重或开训；既有Pi运行状态仍以其运行报告为准。

新路线以完整ABC-130k训练来源为目标，已有96小时乐高子集只用于最初链路验证，不把它当全数据。推荐顺序为数据/环境审计→Qwen无记忆基线→FIFO/反馈关闭/完整RLT消融→完整多任务训练→Thor与闭环验收。观测级RLT为本项目适配方案，在线仅更新隐藏状态和缓存，不自动更新权重；最新作者已有小规模合成任务实验，不能外推机器人效果。
...
**最新用户决定。** 不使用100000初始化；从官方Pi0.5基础权重开始，随机抽取原乐高分拣train的10小时完整示范，联合进行RTC目标训练与全量微调。该决定替代本节先前的100000→2h短适配建议；100000仅保留历史业务基线。这里“重新训练”仍继承Pi0.5预训练模型，不是随机初始化全部参数。实现阶段仅准备代码；随后用户明确授权启动，实际运行见本节末尾交接记录。未同步Thor。
