...
- Deployment success additionally requires `smoke_test_openarm_policy_server.py` to complete one websocket inference with finite `(50,16)` actions and bind the evidence to the selected checkpoint; listening on `6666` is not sufficient.
