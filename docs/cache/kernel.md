...
- **Thor RTC30000当前状态（2026-09-18 09:33 CST复核）**：训练Pi0.5用分位数归一化，旧服务/JAX参考曾误用均值/标准差；旧7/10步精度验收失效。修正前缀后，七步FP32权重＋TF32 TensorRT `pi05-rtc-infer` 已在固定 `ws://192.168.250.1:8000` 运行、宿主MAXN临时会话active；握手分位数前缀及0.2rad/tick关节跳变拒绝，旧回执不能启动新版。Thor本机原9例两轮及乐高分拣3事故4例WebSocket通过，有限50×14与前缀保持；事故4例最大新后缀关节步进0.0924rad/tick、本机往返P50约202.6ms。第二轮9例有4次约390ms往返、服务推理仍约193–194ms，原因未定；有`192.168.250.2`连接但**未做3588协议smoke/真机验收**。容器`--restart no`，停止后自动回120W，不是开机常驻。旧10步不是安全回退。证据及在线/离线边界见[事故报告](../reports/thor/rtc-prefix-quantile-incident-20260917.md#2026-09-18-更新新版服务与-thor-本机协议测试)。
