...
- 2026-09-24 多模型控制层 v0.2 已接模型/数据/算法组合配置、源文件 inventory、按采集组的不可覆盖 split 和运行前身份复核；保留 v1 实验与各后端原生训练器，首版仅核心+CLI，多用户 API 未实现。Pi/OpenWAM/XR-1 的模块化训练入口仍待真实 GPU 验收，Evo-1/FastWAM/VLA-JEPA 未开放。动作语义不兼容直接拒绝；数据、操作与限制见 [10](../10_vla_platform.md#模块化配置后端-v02)，架构见 [01](../01_system_architecture.md#多模型接入层)。
- 2026-09-24 XR-1 官方 5B 后训练 checkpoint 与独立 Conda 环境已放在服务器；`pip check` 通过。本地固定上游源码、`xr1` 后端及原生训练入口已接入，YAM 派生末端标签/FK、统计与部署 IK 尚缺，未运行训练或 GPU 验收。原生 state 是关节/夹爪，action 是末端局部相对位姿/夹爪并含 waist/base 槽；不能将 YAM joint-space 14D action 直接投影。资产摘要见 [环境报告](../reports/environments/xr1-20260924/README.md)，操作见 [10](../10_vla_platform.md#2026-09-24--xr-1-原生训练入口)。
...
- Thor 已完成官方系统安装并从 NVMe 启动；2026-09-07 实机复核为 JetPack 7.2.1 / L4T 39.2.1。2026-09-23 工作站与 Thor 曾落在不同 AP/网段；Thor 新增工作站 AP 优先连接，当时 DHCP `10.18.10.89/23`、本机 SSH/NoMachine 端口可达，**非静态 IP、重启回连未验**。旧5 GHz/2.4 GHz配置保留回退；Thor↔3588生产直连为 `192.168.250.1↔192.168.250.2`，连通曾实测通过。3588 Wi-Fi 的2026-09-24最近实测地址为 `10.18.10.39/23`，也是 DHCP，不能沿用更早的地址；生产链路不依赖两者Wi-Fi互通。USB 管理口 `192.168.55.1`。HDMI 仍无显示，不能误记为安装未完成；详见 [Thor 当前状态](../08_thor_edge_deployment.md#7-当前状态)。
