...
- Thor 训练时 RTC 推理已有独立候选代码/镜像：干净前缀 `t=0`、动态 mask、JAX→Torch→TensorRT 数值 gate 与30Hz policy tick协议。2026-09-17 服务器20000保存点已完整传至Thor，17个params/assets文件与元数据/训练合同两端SHA一致；原始参数非有限0，FP32转换811张量加载逐位一致；9组真实d=0/1/10离线回放已生成。训练源/保存点norm仅差末尾换行、数值字节相同。用户要求旧推理测试期间RTC只转换、不测试；ONNX导出依赖GPU上的同输入数值对照，故真权重动作对照/engine/延迟均待后续授权，100000普通服务不变；`thor-rtc-20000`心跳已暂停。详见 [RTC 冷手册](../reference/thor/13_trained_rtc_inference.md)。
