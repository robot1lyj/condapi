...
- Thor 训练时 RTC 推理已形成独立候选代码：干净前缀 `t=0`、动态前缀 mask、JAX→Torch→TensorRT 数值 gate 与严格 tick 协议；仅 Thor 隔离 CPU 合约测试 6/6 通过，真实 RTC checkpoint/engine/延迟尚未验收，100000 普通服务保持不变。详情和命令见 [RTC 冷手册](../reference/thor/13_trained_rtc_inference.md)。
