...
- Pi0.5 原始 JAX 基础权重、分词器和 3 条完整 YAM 回放轨迹均在 Thor 本地。2026-09-07 晚间已完成 17 组配置、3060 次正式调用。当前最快 V：TensorRT BF16/FP32 + 固定时间 FP32 AdaRMS 缓存 + CUDA Graph，P50 108.85 ms / P95 109.52 ms；对 JAX A 动作 MAE 0.002518、最大差 0.017523（数据集单位）。缓存准备对原 PyTorch 9 输入零差异，V 图/非图零差异，但新旧引擎 U/V 不完全相同，不能称为全链路零误差或任务精度保证。保留 JAX FP32 参考与简单 PyTorch I 回退；尚未达到 ≤100 ms。下一步独立评估仅去除 mask=false 文本填充，必须保留实际 token 并准备长输入回退。所有已测组保持三相机、文本 200、H50、去噪 10，基础模型无 LoRA；测试结束恢复 120W。详见 [加速执行记录](../reference/thor/10_acceleration_execution.md) 与 [中文测试报告](../reports/thor/index.html)。
