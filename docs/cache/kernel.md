...
- **2026-09-17 晚间 RTC 事故**：训练Pi0.5用分位数归一化，旧Thor服务与旧JAX参考却把RTC绝对前缀编码为均值/标准差；旧7步/10步等JAX↔TRT验证及协议smoke不再证明RTC条件正确。最新乐高分拣3逐帧为RTC（manifest TDA是热切换未更新）；4次回复前后缀关节交界现场最高2.505rad。正确分位数下同输入固定噪声JAX/TRT最高交界约0.104rad；9例JAX d10错误交界0.71–1.11rad降至0.04–0.05rad。新9例JAX↔TRT最大单关节差0.00452rad（动作49），事故4例最大0.00212rad，均不代表真机精度。`pi05-rtc-infer` **已停**、8000无监听、Thor120W；新服务需要正确前缀验证回执并设0.2rad/tick粗大跳变拒绝，尚未重启或做新协议/真机验收。旧10步也不能直接回退。详见[事故报告](../reports/thor/rtc-prefix-quantile-incident-20260917.md)。

## 恢复任务

- 100000 W/80 的既有 `pi05-infer` 曾在固定 `ws://192.168.250.1:8000` 完成本机smoke，MAXN单次约106ms；2026-09-17 已停止。之后的RTC30000七步服务亦因前缀归一化事故停止，**固定8000当前无服务**。以后推理/测试阶段均MAXN、其他阶段120W；当前Thor已回120W。恢复必须用正确分位数编码、新验证回执与受控协议/真机闸门，不能只启动历史容器。详见[事故报告](../reports/thor/rtc-prefix-quantile-incident-20260917.md)，100000历史转换归[交接](../reference/thor/12_checkpoint_handoff.md)。
