...
- Thor 训练时 RTC 推理已有独立候选代码/镜像：干净前缀 `t=0`、动态 mask、JAX→Torch→TensorRT 数值 gate 与30Hz policy tick协议。2026-09-17 同一run的20000已传至Thor，原始参数非有限0、FP32转换811张量逐位一致；30000已传至Thor且params/assets逐文件SHA一致，尚未审计/转换。9组真实d=0/1/10离线回放已为20000生成。用户要求旧推理测试期间不做RTC动作测试；ONNX导出依赖GPU同输入数值对照，engine/延迟仍待后续。100000普通服务当日重载并返回健康OK，原权重/引擎/端口不变、宿主MAXN；`thor-rtc-20000`心跳仍暂停。详见 [RTC 冷手册](../reference/thor/13_trained_rtc_inference.md)。
