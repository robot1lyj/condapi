...
- Thor RTC 30000 固定 `ws://192.168.250.1:8000` 已按用户要求切为**7步FP32权重＋TF32/80-token/时间缓存/CUDA Graph**：2026-09-17 18:40 CST MAXN本机9/9真实WebSocket smoke通过，有限绝对目标50×14、`d=0/1/10`前缀逐位保持，握手有7步及三重指纹；服务/本机往返P50约194/195ms，首请求约212ms，不能宣称3588全链路≤200ms。7步离线同配置JAX→TRT最大单关节差0.000888rad；相对10步JAX的采样变化最大0.018445rad，两类误差不能混同。10步旧服务约1033ms为历史，容器 `pi05-rtc-infer-fp32-10step-preserved-20260917` 已停止保留；旧100000也停止，8001预检已停。当前临时MAXN unit随运行容器监视，重启后需重建，不是开机持久服务。5/6/8/10步及BF16仍为离线候选；FP8仅调研，优先真实数据PTQ而非改FP8微调。3588直连/真机闭环未知。当前状态、回执与回退见 [RTC冷手册](../reference/thor/13_trained_rtc_inference.md)。

## 恢复任务

- 100000 W/80 的既有 `pi05-infer` 曾在固定 `ws://192.168.250.1:8000` 完成本机smoke，MAXN单次约106ms；2026-09-17 已按用户要求停止，不可再按历史记录视为运行中。固定地址现在由RTC30000**7步**服务提供，本机往返P50约195ms，尚未通过3588直连和真机验收。以后推理/测试阶段均MAXN、其他阶段120W；RTC MAXN临时会话跟随容器运行，容器停止后恢复日常模式；机器重启后两者不自动恢复。当前握手、指纹、运行/电源检查和回执归[05](../05_inference_and_rollout.md#2026-09-17--固定8000的-rtc-30000-服务)，100000历史转换与离线对照归[交接](../reference/thor/12_checkpoint_handoff.md)。
...
- Thor 已完成官方系统安装并从 NVMe 启动；2026-09-07 实机复核为 JetPack 7.2.1 / L4T 39.2.1。2026-09-17 管理 Wi-Fi 已从2.4 GHz切到`琶洲模方`5 GHz，DHCP仍为`192.168.110.250`、自动连接优先级200（重启回连待验），旧2.4 GHz配置保留回退；SSH密钥登录可用，3588直连`192.168.250.1/24`未改。USB `192.168.55.1` 为临时管理/传输通道。HDMI 仍无显示，不能误记为安装未完成；详见 [Thor 当前状态](../08_thor_edge_deployment.md#7-当前状态)。
