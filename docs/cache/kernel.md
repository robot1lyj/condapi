...
- 100000 W/80 的既有 `pi05-infer` 曾在固定 `ws://192.168.250.1:8000` 完成本机smoke，MAXN单次约106ms；2026-09-17 已按用户要求停止，不可再按历史记录视为运行中。固定地址现在由RTC30000服务，P50约1033ms，尚未通过3588直连和真机验收。以后推理/测试阶段均MAXN、其他阶段120W；新的RTC MAXN会话跟随容器运行，容器停止后恢复日常模式。握手、指纹、运行/电源检查和回执归[05](../05_inference_and_rollout.md#2026-09-16--100000-服务已启动)，100000转换与离线对照归[交接](../reference/thor/12_checkpoint_handoff.md)。
