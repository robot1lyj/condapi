...
- Thor 训练时 RTC 推理已有独立候选代码/镜像：干净前缀 `t=0`、动态 mask、JAX→Torch→TensorRT 数值 gate 与30Hz policy tick协议。2026-09-17 服务器20000保存点已提交，Thor续传中；9组真实d=0/1/10离线回放已生成。训练源/保存点norm仅差末尾换行、数值字节相同。尚未完成真权重推理/engine/延迟，100000普通服务不变；续传心跳`thor-rtc-20000`。详见 [RTC 冷手册](../reference/thor/13_trained_rtc_inference.md)。
