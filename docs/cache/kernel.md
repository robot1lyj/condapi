...
- Pi0.5 原始 JAX 基础权重、分词器和 3 条完整 YAM 回放轨迹均在 Thor 本地。2026-09-07 23:08 已完成 16 组配置、2880 次正式调用：最快 L 组 P50 118.85 ms，但有旧循环数值分歧；保留更简单的 I 组约 124.15 ms / 对 JAX A 动作最大差约 0.0161。非量化 BF16/FP32 TensorRT 已实测，T 常规执行 129.13 ms、U 图重放 127.37 ms；T/U 逐输入 14D/32D 输出零差异，对 JAX A 最大差约 0.01715。约 100 ms 且保精度目标未完成，下一步为真实输入算子分析与融合注意力适配，不再盲调调度或默认 FP4。所有组固定三相机、H50、去噪 10，只是基础模型录像回放，不是任务精度保证；结束已恢复 120W。详见 [加速执行记录](../reference/thor/10_acceleration_execution.md) 与 [中文测试报告](../reports/thor/index.html)。
