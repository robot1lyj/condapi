...
| training-time RTC、基础权重、随机10小时子集 | [JAX实现与启动](../03_training_and_evaluation.md#2026-09-16--pi05-training-time-rtc基础权重与随机10小时)；前缀条件、新run、子集重算norm、完整episode清单 | `train_lego_full.py` 与 `select_lego_rtc_episodes.py`；本地仅轻量/抽象形状验收，真实GPU训练与trained RTC部署未验收 |
