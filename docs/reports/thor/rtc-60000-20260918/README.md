...
## 后续上线（2026-09-18 11:14 CST）

用户明确授权后已切换固定`ws://192.168.250.1:8000`，`pi05-rtc-infer`加载本报告60000七步引擎/权重，MAXN会话active。九例真实[在线协议检查](online-smoke.json)通过，握手指纹匹配、quantile前缀、有限50×14及前缀保持；服务推理P50/P95=197.16/207.03ms，本机往返197.85/208.18ms（短样本含首请求）。30000停止保留用于回退，未操作3588；真机任务效果待用户测试。以下“60000未上线”仅描述此前离线阶段。
