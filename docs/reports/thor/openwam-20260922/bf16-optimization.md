...
另一个值得验证的**精确复用候选**：Alpha TI2V 把第一帧潜变量在每个去噪步固定，[固定版本的时间条件代码](https://github.com/OpenWAM-Official/OpenWAM/blob/90e94ae31efddd64b59e00365cfc501d9a972eb1/openwam/model/video_backbone/wan/dit_forward.py)把该帧时间条件钉在0；[视频掩码](https://github.com/OpenWAM-Official/OpenWAM/blob/90e94ae31efddd64b59e00365cfc501d9a972eb1/openwam/model/video_backbone/wan_backbone.py)与[跨模态掩码](https://github.com/OpenWAM-Official/OpenWAM/blob/90e94ae31efddd64b59e00365cfc501d9a972eb1/openwam/model/architectures/utils/mask_modes.py)均禁止首帧查询看到未来视频或动作。这里查的是Thor实际固定的 `90e94ae` 版本，不以本仓库另一份 `7c5861e` 厂商快照替代。因此，给定相同观测/prompt，一次请求内首帧分支的逐层状态理论上不依赖去噪步。入口 [`probe_prefix_invariance.py`](../../../../scripts/thor/openwam/probe_prefix_invariance.py) 已准备比较首次与第二次完整前向的首帧输入及每层输出。**尚未在Thor执行，不能声称首帧缓存可用或能省多少毫秒。** 必须先证实逐层输出相等，再实现仅缓存首帧 K/V 与残差的分块前向，最后以原完整模型输出逐维比较；不能只缓存第一层或忽略动作查询对首帧 K/V 的读取。
