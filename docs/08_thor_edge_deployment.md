...
2026-09-23 13:36 CST 经Thor直连 SSH **只读查询**时，3588 的 `wlan0` DHCP 地址位于旧 `192.168.110.0/23` 网段；这是历史快照，不是当前登录地址。2026-09-24 从工作站经已信任的同一 SSH 主机密钥复核：3588 `wlan0=10.18.10.39/23`，`linux@10.18.10.39` 可登录，面板 `http://10.18.10.39:8766/` 返回 HTTP 200；`lan1=192.168.250.2/24` 未变。3588 的 Wi-Fi 地址由 DHCP 分配，使用前重新核对；Thor 与3588的生产通信仍走独立直连网口，不依赖 Wi-Fi 地址。
