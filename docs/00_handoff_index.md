...
| 02 | 服务器与 Thor 环境 | conda、远端资源、数据预检、训练路径、Thor 系统入口和安全要求 |
| 03 | 训练与评估 | YAM config、norm、训练、checkpoint 和评估 gate |
| 04 | 数据合同 | LeRobot、YAM 14D、图像/动作键、单位待核项和 norm |
| 05 | 端侧/IPC policy smoke | policy 输入输出协议、Thor 本地推理和 Thor↔3588 网络验收，不承载机械臂控制说明 |
| 06 | 历史研究 | OpenArm/KAI0/Evo-RL 旧计划，仅供追溯 |
| 07 | 变更历史 | 按日期记录原因、结果和事故 |
| 08 | Thor 端侧部署 | 官方系统、容器、Pi0.5 转换/加速和端侧验收，不承载机械臂驱动说明 |
| 09 | 记忆系统 | 按需检索、计量边界、证据生命周期与 skill 接入 |
| 10 | 多模型接入操作 | Conda、插件操作、工作树使用和模型接入状态 |

## 状态词

Thor 现场安装交给另一个 agent 时，从 [Thor 安装系列 00](reference/thor/00_start_here.md) 读取交接提示，再按当前 G 阶段加载一份操作冷手册；不要将整组加载到热记忆。版本与精度决策仍归 [08](08_thor_edge_deployment.md)。
