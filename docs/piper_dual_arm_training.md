...
本说明基于 `docs/offline_docker_deploy.md` 的离线 Docker 部署流程，并结合本地双臂数据集示例 `/home/lyj/pen`。
...
## 3. 双臂 Piper 配置要点
针对 `/home/lyj/pen`，在 `LeRobotPiperDataConfig` 中建议设定：
...
确保数据集在容器内可见，例如：
```bash
scripts/docker/run_dev.sh --gpus all --bind /home/lyj/pen:/data/pen -- /bin/bash
```
