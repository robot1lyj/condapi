...
## 2026-09-17 · 制定Qwen3-VL-2B与RLT独立记忆方案

- 用户选定Qwen3-VL-2B、确认Recurrent Looped Transformer论文身份，并要求第一数据源为YAM ABC-130k。核对官方模型/数据metadata、RLT最新修订和固定Evo代码，区分完整多任务数据与现有乐高子集、状态递推与在线权重学习。
- 新增[详细方案](reference/qwen3_vl_rlt_abc130k_plan.md)：原生Qwen特征接入、观测级有界RLT、连续序列/burn-in/TBPTT、两类缓存一致性、无记忆/FIFO/无反馈消融、分阶段预算、四卡资源估算、FlashAttention实测要求、Thor状态协议和WP0–WP6。研究假设和候选超参数明确标为未验证。
- 文档在独立研究工作树完成，复用既有训练与接入owner路由；未实现新policy、未下载新权重、未启动训练或修改Thor/3588。后续从数据盘点与Qwen基线开始。
