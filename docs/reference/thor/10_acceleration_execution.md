...
W 保留 V 的原 FP32 时间条件缓存、BF16/FP32 strongly typed 非量化、TF32 关闭和 CUDA Graph，只消除文本桶 80 之后的无效填充。完整 tokenizer 接口 200，实际 token、三相机、H50、去噪 10 均保留。运行入口拒绝长有效文本，须显式走 V；未实现服务层自动回退。图/非图 9/9 零差异、180 次重复输出零变化。GPU 实测 1572–1574 MHz、最高 56.312°C；全进程系统 RAM 峰值 15791 MiB（含 OS/桌面），供电观测峰值 139.762W，不把 MAXN 模式名当成功耗读数。

ONNX `pi05-onnx-text80-20260908-r1` 用时 722.35s，engine `pi05-trt-text80-20260908-r1` 构建命令用时 475.60s（TRT 内部 engine build 464.15s）。引擎 SHA `a0052cf5847f565d804fe633f4445cd58037bf19e094704286986498775b1e78`，约 6009 MiB。导出代码 `008db2b`，构建/回放代码 `849c8c8`；镜像 v6 不变。全部正常退出并恢复 120W/动态频率/自动风扇。

累计 **18 组有效配置 / 3240 次正式调用**；三次准备诊断与 27 次 profiler 调用另列。W 比 V 快 4.59ms，在本回放样本的 MAE/最大差汇总上没有扩大，但不推导为任务成功率或未来 LoRA 精度保证。当前固定 W 为约 100ms 的性能候选，保留 JAX A 和原 V/I；严格 ≤100ms 尚未达到，不默认进一步 FP8/FP4 量化。复现入口与后续验收见 [固定候选方案](11_pi05_candidate_test_plan.md)。

证据：`docs/reports/thor/evidence/20260908/acceleration/`，包括 W 的 result、对 JAX A/对 V 比较、退出/频率/温度日志，以及导出/引擎报告。原始数组仍保留在两机对应 `results/pi05-W-20260908-r1/`，大模型文件不入 Git。

## 2026-09-08 · 文本填充诊断（不是新测速成绩）

基于 `148a0e9` / v6 镜像完成三个独立诊断，9 个原始 YAM 输入、固定噪声、三相机/H50/10 步不变：

| 运行 | 对照 | 原始 32D 最大差 | 原严格 FP32 门槛 |
|---|---|---:|---|
| `pi05-prepare-text200-fp32-20260908-r1` | FP32 原采样器对时间缓存 wrapper | 0 | 9/9 |
| `pi05-prepare-text80-fp32-20260908-r1` | FP32 原 200 桶对 text80 | 2.712011337e-6 | **8/9，未全通过** |
| `pi05-prepare-text80-bf16-20260908-r1` | BF16 原 200 桶对 text80 | 0.00279968977 | 不适用 |

全部 finite、正常退出、恢复 120W。FP32 text80 最坏动作单位差 1.3689888578e-6；BF16 为 0.0026155620。FP32 原门槛 `rtol=1e-5, atol=1e-6` 的失败点在 episode97-late 的右夹爪（维度 13），不能按无效维度忽略。实际有效 token 数 64–70；mask/位置映射单元测试证明仅移除无效填充时有效注意力边与位置完全保留，但不保证浮点执行结果逐位一致。

代码 `008db2b` 开始新的**离线** text80 ONNX 实验，使用预先存在的 PyTorch FP32 默认数值比较作为附加诊断，记录原严格 8/9 结果，不据此放行任务精度。对照分层：同桶 eager→wrapper 必须完全一致；原 200 桶→text80 的变化独立记录；TensorRT 最终再对原 JAX A。详见 [执行方案](11_pi05_candidate_test_plan.md)。截至启动时没有新的完整延迟，V 的 108.85 ms 仍是最快已完成成绩。

小型原始证据归 `docs/reports/thor/evidence/20260908/acceleration/`；完整 NPZ、噪声及缓存保留在工作站 `thor-system/test-data/artifacts/<run_id>` 与 Thor `thor/pi/artifacts/<run_id>`，不删除或覆盖。
...
历史记忆计量：当时账本 `thor-accel-20260907-c3` 记录 11678 UTF-8 字节。2026-09-11 已取消旧累计额度及压缩后预留预算的操作规则；此数字仅供追溯，当前检索和迁移规则归 [记忆系统](../../09_memory_system.md)。完整请求 tokenizer/封装对宿主不可见，不能宣称这是完整请求 token 限制。详细日志与动作数组不因记忆预算而删除。
