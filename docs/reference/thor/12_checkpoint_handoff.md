...
2026-09-18起接入技能提供**普通 / 训练时RTC**两种选择。本页第1–5节持有普通W路线；训练合同`rtc_training_max_delay>0`时转到[RTC接入](13_trained_rtc_inference.md)，不能直接套用本页BF16主计算方案。优先遵循用户指定模式，未指定则根据实际训练合同选择；参数形状和转换器默认配置不足以识别RTC。以下100000启动命令为历史记录，现场当前服务状态以[05](../../05_inference_and_rollout.md)为准。
