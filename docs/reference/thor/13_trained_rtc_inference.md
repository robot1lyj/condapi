...
2026-09-17 晚间最新状态：**30000 RTC 旧服务因前缀归一化错误已停止，`ws://192.168.250.1:8000` 当前无监听；Thor 已恢复120W。**旧 JAX↔TensorRT 验收与 WebSocket smoke 是错误前缀条件下的历史结果，不得作为当前 RTC 可用性证据。根因线索、正确分位数编码下的4例事故＋9例离线重测、临时粗大跳变保护及恢复闸门见[事故报告](../../reports/thor/rtc-prefix-quantile-incident-20260917.md)。下文早期上线/性能记录均为历史，不表示服务还在运行。旧100000与10步容器均保留但停止，**旧10步也有同一前缀错误，不是安全回退方案。**
