...
## 2026-09-23 · 20h136000 已替换 124000

15:22 CST 现场复核：`pi05-rtc-infer` 已运行 20h136000，固定 `ws://192.168.250.1:8000`，`healthz=OK`，MAXN 会话 active；七步 FP32 权重＋TF32 TensorRT、quantile 前缀、时间缓存/CUDA Graph 不变。18 项源 SHA 与无 NaN/Inf 审计通过，811 张量 FP32 转换逐位一致；九例同输入 JAX/TRT 最大关节差 0.000747 rad、夹爪最大差 0.000320，上线后九例 WebSocket `(50,14)` 有限且前缀保持，服务推理 P50/P95=197.87/207.56ms，本机往返=198.73/208.77ms。仅 Thor 本机/离线验收，3588 端到端及真机效果仍待测试。

136000 源未附独立训练合同，使用同 20h run 的 124000 合同副本，norm 逐字节一致；原始 124000 的 Thor 大权重/引擎及旧容器已按授权清理，释放约 48.36GiB，不能就地回退。服务器源不因本次清理改变。完整身份、回执、恢复边界见 [136000 报告](../../reports/thor/rtc-20h-136000-20260923/README.md)。以下 124000 在线和更早“进行中”章节均为历史快照。

## 2026-09-22 · 20h124000已上线，90000/30000大权重已清理

11:09 CST复核：流水线正常结束，同名 `pi05-rtc-infer` 已运行124000七步FP32＋TF32，固定 `ws://192.168.250.1:8000`、MAXN active、healthz OK。九例同输入JAX/TRT对照与上线后九例协议均通过；最大关节差0.000616rad、夹爪差0.000359，稳态推理P50/P95=191.47/192.40ms，本机协议往返197.42/208.16ms。不是跨3588/真机任务验收。

按用户授权，在新服务验收后永久删除20h90000原始params/转换权重/七步ONNX大权重/TRT引擎及10h30000剩余params，释放约59.96GiB，现可用约537GiB；服务器原件、小报告/norm/配置及其他模型保留。旧模型须重新下载/构建才能回退。完整指纹、原始回执和恢复路径见[124000报告](../../reports/thor/rtc-20h-124000-20260922/README.md)。下文进行中与90000在线均为历史状态。
