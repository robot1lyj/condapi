# Layered storage and hot-memory maintenance

Use when setting entrypoint budgets, consolidating memory or moving navigation. Keep project facts in the project's existing owners, not this skill.
...
Canonical truth and retrieval temperature are independent: a rarely used hardware identity remains authoritative at its owner. Demotion changes loading frequency, not confidence/status. Reuse or verify it under its original scope and freshness conditions. Essential authorization and actuator/data-loss boundaries must remain discoverable before an applicable action.
...
1. Update the fact/procedure at its owner with date, scope, evidence and current status. Use existing prose when sufficient; do not create a record schema or service solely for one sentence.
