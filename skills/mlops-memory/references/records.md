...
- Inputs: dataset manifest hash, split hash, contract/unit audit, transform/config hash, norm hash, base checkpoint identity, LoRA settings, seed.
...
For condapi, training uses W&B disabled and existing local `metrics/metrics.jsonl`, CSV, plots and Slurm logs. Do not enable offline W&B merely to satisfy memory collection. Link dataset → norm/transform → training → checkpoint → conversion → engine → validation. Quantization validation and production authorization are distinct states.
